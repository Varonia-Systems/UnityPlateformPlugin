...
{
    [CustomEditor(typeof(WeaponOffsetCalibrator))]
    public class WeaponOffsetCalibratorEditor : Editor
    {
        // ── Style cache ───────────────────────────────────────────────────────────
        static bool     stylesBuilt;
        static GUIStyle headerStyle, sectionStyle, footerStyle, buttonStyle, tagStyle;
        static GUIStyle fieldLabelStyle, readOnlyStyle, dropdownStyle;
        ...
        static readonly Color colBlue        = new Color(0.30f, 0.65f, 1.00f, 1f);
        static readonly Color colBlueDim     = new Color(0.30f, 0.65f, 1.00f, 0.15f);
        ...
        // ── Textures ──────────────────────────────────────────────────────────────
        static Texture2D texBtn, texBtnHover, texAccentSolid, texWarnSolid, texBlueSolid;
        static Texture2D texCard, texAccentDim, texWarnDim, texBlueDim, texDivider;
        ...
        private void AutoFindLibrary()
        {
            var cal = (WeaponOffsetCalibrator)target;
            if (cal.ghostLibrary != null) return;

            var guids = AssetDatabase.FindAssets("t:WeaponGhostLibrary");
            if (guids.Length == 0) return;

            var path = AssetDatabase.GUIDToAssetPath(guids[0]);
            var lib  = AssetDatabase.LoadAssetAtPath<WeaponGhostLibrary>(path);
            if (lib == null) return;

            cal.ghostLibrary = lib;
        ...
        void BuildStyles()
        {
            ...
            texBlueDim     = MakeRoundedTex(32, 32, colBlueDim, 6);
            ...
            texBlueSolid   = MakeRoundedTex(32, 32, colBlue, 5);
        ...
        public override void OnInspectorGUI()
        {
            ...
            var propIndex     = serializedObject.FindProperty("selectedIndex");
            var propColor     = serializedObject.FindProperty("ghostColor");
            var propWireColor = serializedObject.FindProperty("wireColor");
            ...
            GUILayout.Space(12);
            GUILayout.Label("WEAPON OFFSET CALIBRATOR", headerStyle);
            GUILayout.FlexibleSpace();
            GUILayout.Label("GIZMO", tagStyle);
            GUILayout.Space(12);
            ...
            DrawCard(() =>
            {
                DrawSectionLabel("GHOST LIBRARY");
                DrawDivider();
                GUILayout.Space(6);

                // Dropdown de sélection du ghost
                var library = cal.ghostLibrary;
                if (library != null && library.entries != null && library.entries.Count > 0)
                {
                    string[] names = new string[library.entries.Count];
                    for (int i = 0; i < library.entries.Count; i++)
                        names[i] = string.IsNullOrEmpty(library.entries[i].friendlyName)
                            ? $"Entry {i}"
                            : library.entries[i].friendlyName;

                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("Active Ghost", fieldLabelStyle, GUILayout.Width(120));
                    int newIndex = EditorGUILayout.Popup(propIndex.intValue, names, dropdownStyle);
                    if (newIndex != propIndex.intValue)
                ...
                else
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Space(8);
                    if (cal.ghostLibrary == null)
                        GUILayout.Label("Aucun WeaponGhostLibrary trouvé dans le projet.\nCréez-en un via Assets > Create > VBO > Weapon Ghost Library", readOnlyStyle);
                    else
                        GUILayout.Label("La library est vide — ajoutez des entrées dans le SO.", readOnlyStyle);
                    EditorGUILayout.EndHorizontal();
                }
            ...
            DrawCard(() =>
            {
                DrawSectionLabel("GIZMO");
                DrawDivider();
                GUILayout.Space(6);

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Ghost Color", fieldLabelStyle, GUILayout.Width(120));
                EditorGUILayout.PropertyField(propColor, GUIContent.none);
                EditorGUILayout.EndHorizontal();

                GUILayout.Space(4);

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Wire Color", fieldLabelStyle, GUILayout.Width(120));
                EditorGUILayout.PropertyField(propWireColor, GUIContent.none);
                EditorGUILayout.EndHorizontal();
            ...
            GUILayout.Space(8);
            GUILayout.Label("Varonia Back Office  ·  WeaponOffsetCalibrator", footerStyle);
            GUILayout.Space(8);
